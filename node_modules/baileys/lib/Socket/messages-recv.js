    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            await messageMutex.mutex(async () => {
                await decrypt();
                // YUTA_INVISIBLE_RECEIVER_PATCH_V5
                // Normaliza o ciphertext criado pelo modo Só Membros antes do fluxo de retry.
                try {
                    const yutaDecryptFail = encNode?.attrs?.['decrypt-fail'] || node?.attrs?.['decrypt-fail'] || null;
                    const yutaRemoteJid = msg?.key?.remoteJid || node?.attrs?.from || '';
                    const yutaParticipant = msg?.key?.participant || msg?.participant || msg?.key?.participantAlt || node?.attrs?.participant || null;
                    const yutaIsGroup = String(yutaRemoteJid).endsWith('@g.us');
                    const yutaIsCiphertext = msg?.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT;
                    const yutaIsOnlyMembers = yutaDecryptFail === 'hide' && yutaIsGroup && Boolean(yutaParticipant) && yutaIsCiphertext;

                    if (yutaIsOnlyMembers) {
                        msg.stealthMeta = {
                            patch: 'YUTA_INVISIBLE_RECEIVER_PATCH_V5',
                            decryptFail: yutaDecryptFail,
                            encType: encNode?.attrs?.type || null,
                            failedToDecrypt: true,
                            participant: yutaParticipant,
                            messageId: msg?.key?.id || node?.attrs?.id || null,
                            detectedAt: Date.now()
                        };

                        msg.yutaInvisibleMessage = true;
                        // Mantém o evento no fluxo normal sem criar texto visível ou logável.
                        // O index.js reconhece a marca yutaInvisibleMessage e executa a proteção.
                        msg.message = {
                            requestPaymentMessage: {
                                currencyCodeIso4217: 'BRL',
                                amount1000: 0,
                                requestFrom: yutaParticipant
                            }
                        };
                ...
                catch (yutaInvisibleError) {
                    // Falha silenciosa: sem o patch válido, o Baileys segue o comportamento original.
                }
